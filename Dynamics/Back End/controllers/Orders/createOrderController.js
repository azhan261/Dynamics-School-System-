...
var { OrderCreation } = require('../../models/Orders/OrderCreationSchema')
 ...
 router.get('/', (req, res) => {
     var date = new Date()
     console.log(date)
     OrderCreation.find((err, doc) => {
         var checkingDate = doc.endDate
         console.log(checkingDate)
         var checkingDate = doc[0].endDate
         var newDate = new Date(checkingDate)
         console.log(newDate)
         if (!err) { res.send(doc); }
 ...
 router.get("/:id", (req, res) => {
    ...
    OrderCreation.findById(id, (err, todo) => {
      res.json(todo);
    });
  });
 ...
 router.put('/:id', (req, res) => {
    ...
    var emp = {
        ...
        headOfficee:req.body.headOffice,
        startDate:req.body.startDate,
        endDate:req.body.endDate,
        productionDeadline:req.body.productionDeadline,
        deliveryDate:req.body.deliveryDate,
    ...
    OrderCreation.findByIdAndUpdate(req.params.id, { $set: emp}, { new: true }, (err, doc) =>{
        //Checking for errors
 ...
 router.delete('/:id', (req, res) => {
    ...
    OrderCreation.findByIdAndRemove(req.params.id, (err, doc) => {
        if (!err) { res.send(doc); }
